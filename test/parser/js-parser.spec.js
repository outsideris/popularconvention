...
describe('JS-parser >', () => {
  describe('comma >', () => {
    it('check first comma #1', () => {
      const convention = parser.comma(`,fs = require('fs')`);
  ...
  describe('indent >', () => {
    it('check space indent #1', () => {
      const convention = parser.indent(`var a = 1;`);
  ...
  describe('functiondef >', () => {
    it('check function definition followed by no space #1', () => {
      const convention = parser.functiondef(`var a = function() {`);
      expect(convention.nospace).equal(1);
    });

    it('check function definition followed by no space #2', () => {
      const convention = parser.functiondef(`var a = function() { return 1; };`);
      expect(convention.nospace).equal(1);
    });

    it('check function definition followed by no space #3', () => {
      const convention = parser.functiondef(`function a() {}`);
      expect(convention.nospace).equal(1);
    });

    it('check function definition followed by no space #4', () => {
      const convention = parser.functiondef(`a.fn(function() {})`);
      expect(convention.nospace).equal(1);
    });
  ...
  describe('argumentdef  >', () => {
    it('check argument definition with one space #1', () => {
      const convention = parser.argumentdef(`function a( arg1, arg2 ) {}`);
      expect(convention.onespace).equal(1);
    });
    ...
    it('check argument definition with one space #3', () => {
      const convention = parser.argumentdef(`a.fn(function( arg1, arg2 ) {})`);
      expect(convention.onespace).equal(1);
    });
    ...
    it('check argument definition with no space #1', () => {
      const convention = parser.argumentdef(`var a = function(arg1, arg2) {`);
      expect(convention.nospace).equal(1);
    });

    it('check argument definition with no space #2', () => {
      const convention = parser.argumentdef(`var a = function (arg1, arg2) { return 1; };`);
      expect(convention.nospace).equal(1);
    });
    ...
    it('check argument definition with no space #6', () => {
      const convention = parser.argumentdef(`       }//if                                               -`);
      expect(convention.nospace).equal(0);
      expect(convention.onespace).equal(0);
    });
  });
  ...
  describe('literaldef >', () => {
    it('check object literal definition with trace space #1', () => {
      const convention = parser.literaldef(`  init: "value",`);
      expect(convention.tracespace).equal(1);
    });

    it('check object literal definition with trace space #2', () => {
      const convention = parser.literaldef(` init: function() { `);
      expect(convention.tracespace).equal(1);
    });

    it('check object literal definition with trace space #3', () => {
      const convention = parser.literaldef(`{ key: value, key: value }`);
      expect(convention.tracespace).equal(1);
    });
  ...
  describe('conditionstatement  >', () => {
    it('check condition statement with one space #1', () => {
      const convention = parser.conditionstatement(`if ( a = 1) {`);
      expect(convention.onespace).equal(1);
    });

    it('check condition statement with one space #2', () => {
      const convention = parser.conditionstatement(`while ( ture ) {`);
      expect(convention.onespace).equal(1);
    });

    it('check condition statement with one space #3', () => {
      const convention = parser.conditionstatement(`switch ( a ) {`);
      expect(convention.onespace).equal(1);
    });
  ...
  describe('blockstatement >', () => {
    it('check block statement with one space #1', () => {
      const convention = parser.blockstatement(`if (true) { return; }`);
      expect(convention.onespace).equal(1);
    });

    it('check block statement with one space #2', () => {
      const convention = parser.blockstatement(`} else if ( true ) {`);
      expect(convention.onespace).equal(1);
    });
    ...
    it('check block statement with no space #3', () => {
      const convention = parser.blockstatement(`if (true)`);
      expect(convention.nospace).equal(0);
    });
    ...
    it('check block statement at new line #2', () => {
      const convention = parser.blockstatement(`if (true) // comment`);
      expect(convention.newline).equal(1);
    });
  ...
  describe('quotes >', () => {
    it('single quote #1', () => {
      const convention = parser.quotes(`  var foo = 'bar';`);
      expect(convention.single).equal(1);
    });

    it('single quote #2', () => {
      const convention = parser.quotes(`  var foo = '<div id="bar">baz</div>';`);
      expect(convention.single).equal(1);
    });
    ...
    it('single quote #4', () => {
      const convention = parser.quotes(` 'key': 'value' `);
      expect(convention.single).equal(1);
    });

    it('single quote #5', () => {
      const convention = parser.quotes(` 'key': true `);
      expect(convention.single).equal(1);
    });
